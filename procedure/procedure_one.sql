...
BEGIN
    ...
    FOR i IN 200..300 LOOP
            -- 유저 생성
            INSERT INTO faster.p_user (id, created_at, created_by, deleted_at, deleted_by, updated_at, updated_by, name, password, role, slack_id, username)
            ...
            hub_id := '00000000-0000-0000-0000-0000000000' || LPAD(FLOOR(RANDOM() * 17 + 1)::TEXT, 2, '0');
            INSERT INTO faster.p_company (type, company_manager_id, created_at, created_by, deleted_at, deleted_by, updated_at, updated_by, hub_id, id, address, contact, name)
            ...
            FOR j IN 1..100 LOOP
                    -- 랜덤 공급업체 및 상품 선택
                    SELECT id INTO supplier_id FROM faster.p_company WHERE type = 'SUPPLIER' ORDER BY RANDOM() LIMIT 1;
                    SELECT id, price INTO product_id, P_price FROM faster.p_product p WHERE p.company_id = supplier_id ORDER BY RANDOM() LIMIT 1;

                    -- p_order 삽입
                    order_id := gen_random_uuid();
                    INSERT INTO faster.p_order (total_price, created_at, created_by, deleted_at, deleted_by, updated_at, updated_by,
                    ...
                    -- p_order_item 삽입
                    INSERT INTO faster.p_order_item (price, quantity, created_at, created_by, deleted_at, deleted_by, updated_at, updated_by,
                                                     id, order_id, product_id, "name")
                    VALUES (P_price, 10, now(), i, NULL, NULL, now(), i, gen_random_uuid(), order_id, product_id, '상품명');

                    -- p_orderer_info 삽입
                    INSERT INTO faster.p_orderer_info (created_at, created_by, deleted_at, deleted_by, updated_at, updated_by, id, order_id,
